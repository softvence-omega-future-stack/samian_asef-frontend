...
const drivers = [
  ...
  <h1 className="text-xl md:text-2xl leading-6 font-semibold">Driver Management</h1>
              ...
              <tbody>
                {drivers.map((rider) => (
                  <tr key={rider.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-[#2C2F33] text-sm font-normal">{rider.rides}</td>
                    <td className="py-3 px-4 text-[#2C2F33] text-sm font-normal">{rider.cancel}</td>
      ...
      setOpenMenuId(openMenuId === rider.id ? null : rider.id)
                      ...
                      >
                        <MoreVertical/>
                             ...
                             {openMenuId === rider.id && (
    <div ref={dropdownRef} className="absolute right-30  w-32 bg-white border rounded-lg shadow-lg z-50">
      <button
        onClick={() => {
     navigate(`/dashboard/driver-management/profile/${rider.id}`);
        ...
        <Trash2 size={14} /> Delete
          ...
          <div className="flex flex-col md:flex-row gap-2 items-center justify-between mt-6 text-sm">
            <span className="text-[#6B7280] text-sm font-normal ">
              Showing 1-8 of {drivers.length} entries
            </span>
